...
function App() {
  ...
  const cartCount = cart.reduce((sum, line) => sum + line.quantity, 0); const cartTotal = cart.reduce((sum, line) => sum + line.unitPrice * line.quantity, 0); const packCount = Object.values(packCounts).reduce((sum, count) => sum + count, 0); const packDiscount = packDiscountRate(packTarget); const packPriceForChoice = (product: Product) => { const choice = firstChoicePricing(product); const originalPrice = choice.regularPrice ?? product.regularPrice; return originalPrice === null ? null : Math.round(originalPrice * (1 - packDiscount)); }; const packTotal = packProducts.reduce((sum, product) => sum + (packCounts[product.id] ?? 0) * (packPriceForChoice(product) ?? 0), 0);
