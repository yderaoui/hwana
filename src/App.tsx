...
import { ArrowLeft, ArrowRight, Bag, Check, House, List, MagnifyingGlass, Minus, Package, Plus, SquaresFour, Storefront, Trash, WhatsappLogo, X } from "@phosphor-icons/react";
import catalogData from "./data/catalog.json";
import campaignMedia from "./data/campaign-media.json";
import { Language, LocalizedText, Product, ProductSubcategory, subcategoriesByCategory, translations } from "./data/catalog";
...
const whatsappUrl = "https://api.whatsapp.com/send?phone=212689765468&text=Hawana.ma";
...
type OrderPayload = { id: string; customer: Record<string, string>; items: { name: string; color: string; size: string; quantity: number; unitPrice: number; lineTotal: number; image: string; barcode?: string | null }[]; payment: "cash_on_delivery"; subtotal: number; deliveryFee: number; total: number; createdAt: string };

type CatalogFilterCopy = Pick<(typeof translations)[Language]["catalog"], "all" | "women" | "men" | "kids" | "other">;

const filterOptions = (t: CatalogFilterCopy): [Filter, string][] => [
  ["all", t.all],
  ["femme", t.women],
  ["homme", t.men],
  ["enfants", t.kids],
...
const categoryLabels: Record<Filter, LocalizedText> = {
  all: { fr: "Tous", ar: "الكل", en: "All" },
  femme: { fr: "Femme", ar: "نساء", en: "Women" },
  homme: { fr: "Homme", ar: "رجال", en: "Men" },
  enfants: { fr: "Enfants", ar: "أطفال", en: "Kids" },
  autres: { fr: "Collants & chaussettes", ar: "جوارب وكولونات", en: "Tights & socks" },
};
...
const retailSubcategoryLabels: Record<ProductSubcategory, LocalizedText> = {
  ...
  bodies: { fr: "Bodies & caracos", ar: "بودي وكاراكوس", en: "Bodysuits & camis" },
  collants: { fr: "Collants", ar: "كولونات", en: "Tights" },
  chaussettes: { fr: "Chaussettes", ar: "جوارب", en: "Socks" },
  nuisettes: { fr: "Pyjamas & nuisettes", ar: "بيجامات وملابس نوم", en: "Sleepwear" },
  vetements: { fr: "Shorts & leggings", ar: "شورتات وليغنز", en: "Shorts & leggings" },
  "sous-vetements": { fr: "Slips & boxers", ar: "ملابس داخلية", en: "Briefs & boxers" },
  hauts: { fr: "T-shirts & débardeurs", ar: "قمصان وفانيلات", en: "T-shirts & tanks" },
  fille: { fr: "Fille", ar: "بنات", en: "Girls" },
  garcon: { fr: "Garçon", ar: "أولاد", en: "Boys" },
};

const productCategoryLabel = (product: Product, language: Language) =>
...
const elkoAvailableCopy: LocalizedText = {
  ...
  ar: "أساسيات قطنية مريحة ومتوفرة: سراويل داخلية، بوكسرات، فانيلات وقمصان.",
  en: "Clean comfortable cotton basics, available in briefs, boxers, tank tops and T-shirts.",
};
...
const pageCopy = {
  ...
  en: {
    ...
    buildPack: "Build a pack", packLead: "Bring your essentials together and unlock the pack price.", shopTitle: "The HAWANA selection", shopLead: "Choose your piece, color and size, then order directly.", worn: "Worn", product: "Product", cod: "Cash on delivery", delivery: "Delivery fee", total: "Total", previous: "Previous", next: "Next", collection: "HAWANA collection", direct: "HAWANA",
  },
  ar: {
    ...
    buildPack: "كوّن باقتك", packLead: "اجمع قطعك الأساسية واستفد من سعر الباقة.", shopTitle: "تشكيلة HAWANA", shopLead: "اختر القطعة واللون والمقاس، ثم اطلبها مباشرة.", worn: "عند الارتداء", product: "المنتج", cod: "الدفع عند الاستلام", delivery: "مصاريف التوصيل", total: "المجموع", previous: "السابق", next: "التالي", collection: "تشكيلة HAWANA", direct: "HAWANA",
  },
} as const;
...
function App() {
  ...
  const cartCount = cart.reduce((sum, line) => sum + line.quantity, 0); const cartSubtotal = cart.reduce((sum, line) => sum + line.unitPrice * line.quantity, 0); const cartDeliveryFee = cartCount > 0 ? deliveryFee : 0; const cartTotal = cartSubtotal + cartDeliveryFee; const packCount = Object.values(packCounts).reduce((sum, count) => sum + count, 0); const packDiscount = packDiscountRate(packTarget); const packPriceForChoice = (product: Product) => { const choice = firstChoicePricing(product); const originalPrice = choice.regularPrice ?? product.regularPrice; return originalPrice === null ? null : Math.round(originalPrice * (1 - packDiscount)); }; const packTotal = packProducts.reduce((sum, product) => sum + (packCounts[product.id] ?? 0) * (packPriceForChoice(product) ?? 0), 0);
  ...
  const addPack = () => { if (packCount !== packTarget) return; const packId = `pack-${Date.now()}`; packProducts.forEach((product) => { const choice = firstChoicePricing(product); const packUnitPrice = packPriceForChoice(product); for (let index = 0; index < (packCounts[product.id] ?? 0); index += 1) addToCart(product, choice.colorId, choice.size, packUnitPrice, packId); }); setPackCounts({}); setCartOpen(true); };
  const submitOrder = (event: FormEvent<HTMLFormElement>) => {
    ...
    const order: OrderPayload = { id, customer, items, payment: "cash_on_delivery", subtotal: Math.round(cartSubtotal), deliveryFee: cartDeliveryFee, total: Math.round(cartTotal), createdAt: new Date().toISOString() };
  ...
  return <div ref={pageRef} className="site-shell">
    ...
    <a className="whatsapp-float" href={whatsappUrl} target="_blank" rel="noreferrer" aria-label="WhatsApp HAWANA"><WhatsappLogo size={34} weight="fill" /></a>
