...
import { Language, Product, translations } from "./data/catalog";
...
function App() {
  ...
  useEffect(() => setVisibleLimit(12), [filter, search, language]);
  useEffect(() => {
  ...
  return <div ref={pageRef} className="site-shell">
    ...
    <main id="top">
      ...
      <section className="catalog-section section-wrap" id="shop"><div className="section-heading catalog-heading section-reveal"><h2>{c.shopTitle}</h2><p>{c.shopLead}</p></div><div className="catalog-tools"><div className="filters">{([["all", t.catalog.all], ["femme", t.catalog.women], ["homme", t.catalog.men], ["enfants", t.catalog.kids], ["autres", t.catalog.other], ["ALSAMAH", "ALSAMAH"]] as [Filter, string][]).map(([value, label]) => <button className={filter === value ? "active" : ""} onClick={() => setFilter(value)} key={value}>{label}</button>)}</div><label className="catalog-search"><MagnifyingGlass size={18} /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder={c.searchProducts} /></label></div><p className="result-count">{filteredProducts.length} {t.catalog.count}</p>{filteredProducts.length ? <><div className="product-grid">{filteredProducts.slice(0, visibleLimit).map((product) => <ProductCard key={product.id} product={product} language={language} onOpen={setActiveProduct} onAdd={addToCart} />)}</div>{visibleLimit < filteredProducts.length && <div className="load-more"><button className="button secondary" onClick={() => setVisibleLimit((value) => value + 12)}>{t.catalog.loadMore}<Plus size={17} /></button></div>}</> : <div className="empty-state"><Package size={34} /><p>{t.catalog.empty}</p></div>}</section>
    </main>
